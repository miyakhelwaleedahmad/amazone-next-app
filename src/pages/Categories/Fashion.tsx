...
const Fashion = () => {
  ...
  useEffect(() => {
    const fetchFashionProducts = async () => {
      try {
        const menRes = await fetch("https://dummyjson.com/products/category/mens-shirts");
        const womenRes = await fetch("https://dummyjson.com/products/category/tops");

        const menData = await menRes.json();
        const womenData = await womenRes.json();

        setProducts([...menData.products, ...womenData.products]);
      } catch (error) {
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold">Fashion Products</h1>
